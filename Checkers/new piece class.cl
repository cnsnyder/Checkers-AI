      ...
      (let ((item (nth x validlist)))
                (if (eq 'piece (type-of item))
                    (if (not (eq (slot-value item 'owner) (slot-value thePiece 'owner))) ;;If diff teams
                        ...
                        (progn
                          ...
                          (let ((r (+ (- (nth 0 (slot-value item 'location)) (nth 0 (slot-value thePiece 'location))) (nth 0 (slot-value item 'location))))
                            ...
                            (if (and (>= r 0) (<= r 7) (>= c 0) (<= c 7)) ;;if inside bounds
                                (if (eq (boardItemAt theBoard r c) nil) ;; if item at landing location eq nill 
                                    (progn 
                                      (let ((myBoard theBoard) (myMovelist theMovelist) (originalLocation (slot-value thePiece 'location)) (myPiece thePiece) (orgIsKing (slot-value thePiece 'isKing)))
                                        (incf jumpcounter)
                                        ;;(print "found valid jump on piece")
                                        ;;(print thePiece)
                                            ...
                                            (setf (slot-value thePiece 'isKing) orgIsKing)
                                        )
                                      )
                                  (progn
                                    ;;(print "setting xth value to 0 because landing location is full. x:")
                                    ;;(print x)
                                    (setf (nth x myvalidlist) 0)
                                    )
                                  )
                              (setf (nth x myvalidlist) 0)
                              )
                      ...
                      (progn
                        ;;(print "setting xth value to 0 because trying to jump same team. x: ")
                        ;;(print x)
                        (setf (nth x myvalidlist) 0)
                        )
                      )
        ...
        (progn
          ...
          (let ((nonnil (remove nil (remove 0 validlist))))
            (if (= 0 (length nonnil))
              ...
              (progn
                 ;;Other CASE should never land here by our logic. 
                )
              )
            )
          )
